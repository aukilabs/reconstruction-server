...
def evaluate_job(job_folder, truth_portals_path=None, verbose=False, flatten=False):
    ...
    for scan_dir in local_refined_dir.iterdir():
        ...
        compare_portals(unrefined_poses, refined_poses, truth_portal_poses, align=True, verbose=verbose, correct_scale=False, flatten=flatten)
...
def load_refined(refined_manifest_path):
    if refined_manifest_path.exists():
        ...
        return global_refined_poses
    return None

        print(f"Compare accuracy of {len(global_refined_poses)} portals.")
        compare_portals(global_refined_poses, global_refined_poses, truth_portal_poses, align=True, verbose=True, correct_scale=False, flatten=flatten)
    else:
...
def main():
    ...
    parser.add_argument("--flatten", "-f", help="Flatten the poses", action="store_true")
    args = parser.parse_args()
    
    evaluate_job(args.job_folder, args.truth, args.verbose, flatten=args.flatten)
