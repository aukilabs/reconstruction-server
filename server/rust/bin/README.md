...
together the engine from `compute-node-common` with the available runners. The
...
- Initialize telemetry (`compute_node_common::telemetry::init_from_env`) so the
...
- Spawn DDS registration if fully configured, and kick off the engine loop via
  `compute_node_common::engine::run_node`.

## Environment cross-check
The binary defers to the configuration code in `compute-node-common`, so see
that README for exhaustive documentation. At a minimum you will need:
...
## Testing
- Unit tests live alongside the supporting crates (`compute-node-common`,
  runners). There are no binary-specific tests, but the end-to-end happy paths
  are covered by integration tests under `compute-node-common/tests`.
