...
mod tests {
    ...
    fn settings_none_without_required_fields() {
        let cfg = base_cfg();
        assert!(registration_settings(&cfg, &[NOOP_CAPABILITY.into()]).is_none());
    }
    ...
    fn settings_present_when_all_fields_available() {
        ...
        let settings = registration_settings(&cfg, &[NOOP_CAPABILITY.into()]).expect("settings");
        ...
        assert_eq!(settings.capabilities, vec![NOOP_CAPABILITY.to_string()]);
    }
    ...
    fn node_version_falls_back_when_invalid() {
        ...
        let caps = vec![NOOP_CAPABILITY.to_string()];
