use compute_node_common::config::{LogFormat, NodeConfig};
use compute_node_common::dms::client::DmsClient;
use compute_node_common::engine::{run_cycle_with_dms, run_node_with_shutdown, RunnerRegistry};
...
async fn happy_path_poll_run_complete_with_heartbeat_token_rotation() {
    ...
    let reg = RunnerRegistry::new()
        .register(runner_reconstruction_legacy_noop::RunnerReconstructionLegacyNoop::new(0));
...
async fn run_node_uses_siwe_token_and_completes_task() {
    let server = MockServer::start();
    compute_node_common::dds::persist::clear_node_secret().unwrap();
    compute_node_common::dds::persist::write_node_secret("node-secret").unwrap();
    ...
    for runner in
        runner_reconstruction_legacy_noop::RunnerReconstructionLegacyNoop::for_all_capabilities(0)
    {
