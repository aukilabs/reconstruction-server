...
/// Reduce an arbitrary identifier to a safe single path component. Same character
/// class the compute node itself uses when it derives dataset folder names, so a
/// scan name coming from either side sanitizes identically.
