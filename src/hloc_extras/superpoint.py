...
class SuperPoint(BaseModel):
    ...
    def _init(self, conf):
        ...
        if conf.get("compile_network", False):
            if not SuperPoint.compiled:
                SuperPoint.compiled = superpoint.SuperPoint(conf)
                SuperPoint.compiled = SuperPoint.compiled.eval().cuda()
                SuperPoint.compiled.compile()
            self.net = SuperPoint.compiled
        else:
