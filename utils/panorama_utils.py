# The functions here are used to support panorama generation, which is a special case of the overall refinement pipeline. 
# Processing code are refrenced from panarama_sfm.py from colmap repo, and adapted to fit into the overall pipeline.
# The main difference between panorama generation and local refinement is that panorama generation processes each frame 
# (which is a rig containing multiple images) as a whole, while local refinement processes each image independently.
...
class PanoProcessor:
    def __init__(
        ...
        # GPU acceleration support for remap (if available)
        self.use_cuda = cv2.cuda.getCudaEnabledDeviceCount() > 0
        if self.use_cuda:
            print("[INFO] Using CUDA remap for panorama slicing")
    ...
    def _cuda_remap(self, source: np.ndarray, map_x: np.ndarray, map_y: np.ndarray) -> np.ndarray:
        """Remap using OpenCV CUDA if available."""
        if not self.use_cuda:
            raise RuntimeError("CUDA is not available for remap")

        src_gpu = cv2.cuda_GpuMat()
        src_gpu.upload(source)
    ...
    def process(self, pano_name: str):
        ...
        for cam_idx, cam_from_pano_r in enumerate(self.cams_from_pano_rotation):
            ...
            map_xy = np.moveaxis(xy_in_pano, [0, 1, 2], [2, 1, 0])
            map_x = map_xy[0]
            map_y = map_xy[1]

            if self.use_cuda:
                image = self._cuda_remap(pano_image, map_x, map_y)
            else:
                image = cv2.remap(
                    pano_image,
                    map_x,
